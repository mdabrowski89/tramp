...
class FilterStopsProcessorTest {
    ...
    private fun test(actions: List<FilterStopsAction>, expectedResults: List<FilterStopsResult>) {
        val processor = FilterStopsProcessor(timeTableRepositoryMock, ImmediateSchedulerProvider.instance)
        val testObserver = TestObserver<FilterStopsResult>()
    ...
    companion object {
        ...
        private val testDataSets: List<TestDataSet> = listOf(
            // before first stop
            createTestDataSet(testDataSeed, TimeEntry(9, 32), emptyList()),

            // first tram first stop
            createTestDataSet(testDataSeed, TimeEntry(10, 5), listOf(0)),

            // two trams for one stop
            createTestDataSet(testDataSeed, TimeEntry(10, 44), listOf(3)),

            // new tramp start
            createTestDataSet(testDataSeed, TimeEntry(11, 16), listOf(2)),
            createTestDataSet(testDataSeed, TimeEntry(11, 17), listOf(0, 2)),

            // transition between stops
            createTestDataSet(testDataSeed, TimeEntry(11, 24), listOf(2, 3)),
            createTestDataSet(testDataSeed, TimeEntry(11, 25), listOf(2, 4)),

            // transition between stops
            createTestDataSet(testDataSeed, TimeEntry(13, 12), listOf(0, 3)),
            createTestDataSet(testDataSeed, TimeEntry(13, 13), listOf(1, 3)),

            // last tram last stop
            createTestDataSet(testDataSeed, TimeEntry(13, 40), listOf(5)),
            // after last tram
            createTestDataSet(testDataSeed, TimeEntry(15, 17), emptyList())
        )
        ...
        private fun createTestDataSet(
            ...
            val stops = testDataSeed.timeDiffs.mapIndexed { i, _ -> createStop(i) }
            val timeTables = stops.mapIndexed { i, tramStop ->
                tramStop to TimeTable(createTimeEntries(testDataSeed.timeDiffs[i], testDataSeed.initialTimeEntries), false)
            }.toMap()
            val filterQuery = FilterStopsQuery(targetTime, "lineName", stops)
            val expectedStops = expectedStopIndexes.map { stops[it] }
            return TestDataSet(timeTables, filterQuery, expectedStops)
        }
